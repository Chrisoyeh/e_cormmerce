    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col" id="admin-workspace">
      ...
      <nav className="bg-white border-b border-slate-200 px-6 py-4 flex flex-wrap gap-4 items-center justify-between shadow-xs" id="admin-navbar">
        ...
        <div className={`${isMobileMenuOpen ? 'flex' : 'hidden'} md:flex flex-col md:flex-row items-center gap-3 w-full md:w-auto mt-4 md:mt-0`}>
          ...
          <button
            onClick={onLogout}
            className="w-full md:w-auto px-3.5 py-2 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs rounded-xl transition cursor-pointer"
          >
            Exit Workspace
          </button>
        </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-2xl p-6 space-y-4" id="admin-orders-tab">
            ...
            <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
              ...
              <div className="relative flex-1 min-w-[260px]">
                ...
                <input
                  id="ledger-invoice-search"
                  type="text"
                  value={ledgerSearchTerm}
                  onChange={(e) => setLedgerSearchTerm(e.target.value)}
                  placeholder="Search by pupil name, invoice no (e.g. NAZ-INV-...), or reg no..."
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl pl-10 pr-9 py-2.5 text-xs text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#E37180] focus:border-transparent transition shadow-xs"
                />
                {ledgerSearchTerm && (
                  <button
                    type="button"
                    onClick={() => setLedgerSearchTerm('')}
                    className="absolute right-2.5 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition cursor-pointer"
                    title="Clear search"
                  >
              ...
              {/* Action Buttons (QR Scanner & CSV Export) */}
              <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap shrink-0">
                <button
                  ...
                  className="flex-1 sm:flex-initial px-3.5 py-2.5 bg-[#E37180] hover:bg-[#1e2348] text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 shadow-sm transition cursor-pointer"
                >
            ...
            <div className="flex flex-wrap items-center gap-2.5 pt-1">
              ...
              <select
                ...
                className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-2.5 py-1.5 text-xs text-slate-700 dark:text-slate-200 font-bold focus:ring-1 focus:ring-[#E37180]"
              >
              ...
              {(ledgerSearchTerm || ledgerStartDate || ledgerEndDate || ledgerClassFilter !== 'All' || ledgerPaymentFilter !== 'All' || ledgerDispatchFilter !== 'All' || ledgerReceiptFilter !== 'All') && (
                <button
                  ...
                  onClick={() => {
                    ...
                    setLedgerReceiptFilter('All');
                  }}
                  className="px-2.5 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 dark:bg-rose-950/30 dark:hover:bg-rose-900/40 dark:text-rose-300 border border-rose-200 dark:border-rose-800 rounded-lg text-xs font-bold cursor-pointer transition flex items-center gap-1"
                >
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 p-3 bg-slate-50 dark:bg-slate-800/90 rounded-2xl border-2 border-slate-200 dark:border-slate-700 shadow-xs">
              <div className="flex items-center gap-2.5 flex-wrap">
                <button
                  ...
                  className={`px-3.5 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 cursor-pointer shadow-2xs ${
                    selectedOrderIds.length > 0 && selectedOrderIds.length === filteredOrders.length
                      ...
                      : 'bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100 border border-slate-300 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-650'
                  }`}
                  title={selectedOrderIds.length > 0 && selectedOrderIds.length === filteredOrders.length ? 'Deselect all visible invoices' : 'Select all visible invoices'}
                >
                  <CheckSquare className="w-4 h-4 text-[#E37180]" />
                  {selectedOrderIds.length > 0 && selectedOrderIds.length === filteredOrders.length
                ...
                {selectedOrderIds.length > 0 ? (
                  <>
                    <span className="px-2.5 py-1 bg-rose-100 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-300 dark:border-rose-800 rounded-lg text-xs font-extrabold">
                      {selectedOrderIds.length} of {filteredOrders.length} selected
                    </span>
                    <button
                      type="button"
                      onClick={() => setSelectedOrderIds([])}
                      className="px-2.5 py-1.5 text-xs text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white font-bold cursor-pointer underline underline-offset-2"
                    >
                      Clear selection
                    </button>
                  </>
                  ...
                  <span className="text-[11px] text-slate-500 dark:text-slate-400 hidden sm:inline">
                    Tick row checkboxes or click <strong>Select All</strong> to perform bulk actions
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={selectedOrderIds.length === 0 || isBulkDeletingOrders}
                  onClick={handleDeleteSelectedOrders}
                  className={`px-4 py-2 rounded-xl text-xs font-black transition flex items-center gap-2 ${
                    selectedOrderIds.length > 0
                      ? 'bg-rose-600 hover:bg-rose-700 text-white shadow-md cursor-pointer hover:shadow-lg active:scale-95'
                      : 'bg-slate-100 dark:bg-slate-800/60 text-slate-400 dark:text-slate-600 border border-slate-200 dark:border-slate-700/80 cursor-not-allowed opacity-70'
                  }`}
                  title={
                    selectedOrderIds.length > 0
                      ? `Permanently delete ${selectedOrderIds.length} selected invoice(s)`
                      : 'Select one or more invoices to enable bulk deletion'
                  }
                >
                  <Trash2 className={`w-4 h-4 ${selectedOrderIds.length > 0 ? 'text-white' : 'text-slate-400 dark:text-slate-600'}`} />
                        ...
                        <div className="flex flex-col items-center justify-center max-w-md mx-auto space-y-3">
                          ...
                          <div className="space-y-1">
                            <p className="font-bold text-slate-800 dark:text-slate-100 text-sm">
                              {ledgerSearchTerm ? `No invoices found matching "${ledgerSearchTerm}"` : 'No financial or material sales ledger transactions found'}
                            </p>
                            <p className="text-xs text-slate-500 dark:text-slate-400">
                              {ledgerSearchTerm
                                ? 'Try searching by a different pupil name, invoice number (e.g. NAZ-INV-...), or pupil registration number.'
                          ...
                          {(ledgerSearchTerm || ledgerStartDate || ledgerEndDate || ledgerClassFilter !== 'All' || ledgerPaymentFilter !== 'All' || ledgerDispatchFilter !== 'All' || ledgerReceiptFilter !== 'All') && (
                            <button
                              ...
                              className="px-3.5 py-1.5 bg-[#E37180] text-white hover:bg-[#1e2348] text-xs font-bold rounded-xl transition cursor-pointer shadow-xs"
                            >
